...
from notifications.notification_service import NotificationService, DOMAIN
...
def test_should_send_message_to_user(user_notification_1_test):
    with mock.patch('notifications.notification_service.SlackWrapper') as MockSlackWrapper:
        MockSlackWrapper.return_value.get_users.return_value = [user_notification_1_test]
...
def test_should_send_message_to_two_users(user_notification_1_test, user_notification_2_test):
    with mock.patch('notifications.notification_service.SlackWrapper') as MockSlackWrapper:
        MockSlackWrapper.return_value.get_users.return_value = [user_notification_1_test, user_notification_2_test]
        MockSlackWrapper.return_value.send_message.return_value = True
        menu_id = str(uuid.uuid4())
...
def test_should_not_send_message_to_user_when_no_users_in_slack(user_notification_1_test):
    with mock.patch('notifications.notification_service.SlackWrapper') as MockSlackWrapper:
